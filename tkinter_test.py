...
# 设置最上面的图片,这个不是很好弄，先是要弄一张合适的图片，要去修改分辨率
# 905*115比较好
...
# 设置容器 / 分区
# 此处采用 panedWindow 和 LabelFramec 容器控件
...
# 一些必要的工具函数
# sql_query 是负责连接数据库并且执行一些简单操作的函数
def sql_query(sql):
...
# tree_clear 这是用来清空TreeView控件里面的已存在的内容的
def tree_clear(*args):
...
# 这个 <<TreeviewSelect>> 可以判断是否被选中
window.tree.bind('<<TreeviewSelect>>',switchButtonState)
